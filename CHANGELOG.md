...
*  Allow non standard options to be passed to some API calls. These API calls
  are: create_meeting, join_meeting_url, join_meeting, get_recordings.
...
* New method get_api_version that returns the version of the server API (>=
  0.7).
...
* Better exception throwing when the user is unreachable or the response is
  incorrect.
